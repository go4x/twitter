...
func (o *OAuth2FollowApi) Follow(accessToken string, userId, targetUserId string) (FollowRet, error) {
	...
	if err != nil {
		return FollowRet{}, errors.Wrapf(ErrApi, "request error: %v", err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		return FollowRet{}, errors.Wrapf(ErrApi, "request error: %v", resp.Status)
	}
	...
	if err := json.Unmarshal(bs, &result); err != nil {
		return FollowRet{}, errors.Wrapf(ErrApi, "invalid response: %v", string(bs))
	}
